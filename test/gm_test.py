...
def main():
    ...
    if continuex:
        ...
        os.system(f"curl -ks -H 'Authorization: Bearer {token}' 'https://prometheus-k8s-openshift-monitoring.apps.{sys.argv[2]}.corp.domain.ca/api/v1/query?query=openshift_deploymentconfig_created&time={epochend}&_={epochend}000' > dc.json")        
        f = open("./dc.json")
        data = json.load(f)
        f.close()
        dcl = TinyDB('./dcl.json')
        for _data in data["data"]["result"]:
            def datainsert():
                dcl.insert({'dc': _data["metric"]["deploymentconfig"],'ns': _data["metric"]["namespace"],'getmetric':  f'https://prometheus-k8s-openshift-monitoring.apps.{sys.argv[2]}.corp.domain.ca/api/v1/query_range?query=avg(container_cpu_usage_seconds_total%7Bnamespace%3D%27'+_data["metric"]["namespace"]+'%27%2Cpod%3D~%27'+_data["metric"] ["deploymentconfig"]+f'.*%27%7D)%20by%20(namespace)%20&start={epochstart}&end={epochend}&step={step}&_={epochend}000'})        
    ...
    if continuex and os.path.exists(my_path) and os.path.getsize(my_path) > 0:
         ...
         for itemdc in getdc:     
              ...
              for item in metrics:
                  ...
                  counter += 1
              one_liner += "\n"        
         url = "https://confluence.corp.domain.ca/rest/api/content/87198483"
         headers = {
